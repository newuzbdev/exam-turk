...
export default function ListeningPart5({ sections: apiSections }: ListeningPart5Props) {
  ...
  useEffect(() => {
    if (apiSections && apiSections.length > 0) {
      console.log("ListeningPart5 - API Sections:", apiSections);
      const transformedSections = apiSections.map((section) => ({
        ...
        questions: (section.questions || []).map((question, index) => {
          ...
          const questionNumber = question.number || (24 + index);
      ...
      console.log("ListeningPart5 - Transformed Sections:", transformedSections);
