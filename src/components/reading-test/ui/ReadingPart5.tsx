...
export default function ReadingPart5() {
  const [selectedAnswers, setSelectedAnswers] = useState<Record<string, string>>({ q30: "A" });
  const [paragraphAnswers, setParagraphAnswers] = useState<Record<string, string>>({ s33: "SEÇ", s34: "B", s35: "D" });
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                ...
                {multipleChoiceOptions30_32.map((option) => (
                  <div key={option.id} className="flex items-start gap-3 cursor-pointer p-2 rounded" onClick={() => setSelectedAnswers((prev) => ({ ...prev, q30: option.id }))}>
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${selectedAnswers.q30 === option.id ? "bg-green-500 text-white" : ""}`}>{selectedAnswers.q30 === option.id && <div className="w-3 h-3 bg-green-500 rounded-full"></div>}</div>
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <Input value={paragraphAnswers.s33} onChange={(e) => setParagraphAnswers((prev) => ({ ...prev, s33: e.target.value }))} className="w-16 h-8 text-center" />
                  </div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <Input value={paragraphAnswers.s34} onChange={(e) => setParagraphAnswers((prev) => ({ ...prev, s34: e.target.value }))} className="w-16 h-8 text-center" />
                  </div>
                  <p className="text-xs text-gray-600">Elektronik okumanın beynin kavrama yetisi ve dikkat üzerindeki etkisi, hangi paragrafta yer almaktadır?</p>
                </div>
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <Input value={paragraphAnswers.s35} onChange={(e) => setParagraphAnswers((prev) => ({ ...prev, s35: e.target.value }))} className="w-16 h-8 text-center" />
                  </div>
                  <p className="text-xs text-gray-600">Elektronik okumanın beynin kavrama yetisi ve dikkat üzerindeki etkisi, hangi paragrafta yer almaktadır?</p>
                </div>
              </div>
